...
function EditResource() {
  ...
  const fetchResource = async () => {
    try {
      ...
      // The API might return quantity/capacity as numbers, make sure they are set correctly
      setResource({
        name: res.data.resource.name || "",
        type: res.data.resource.type || "",
        quantity: res.data.resource.quantity || "",
        location: res.data.resource.location || "",
        capacity: res.data.resource.capacity || "",
      });
    } catch (error) {
      toast.error("Unable to load resource");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await api.patch(`/resources/${id}`, resource, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="max-w-2xl mx-auto w-full relative" style={{ paddingBottom: '40px' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <PageHeader 
          title="Edit Resource"
          icon={Edit3}
          description="Update the details and availability of this resource."
        />
      </div>

      {loading ? (
        <div className="text-center py-[80px] text-[#94A3B8]">Loading resource details...</div>
      ) : (
        <Card noPadding className="border border-[#273449]/50 bg-[#111827] shadow-[0_8px_30px_rgba(0,0,0,0.12)]" style={{ padding: '40px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
            ...
            <div className="flex justify-end gap-[16px] pt-[16px] mt-[8px] border-t border-[#273449]/50">
              <button
                type="button"
                onClick={() => navigate("/admin/resources")}
                className="h-[48px] px-[32px] rounded-[16px] text-white text-[15px] font-[600] border border-[#273449] hover:bg-[#1e293b] transition-all duration-300"
              >
                Cancel
              </button>
              <button
